...
export function rollCounters(c: Counters, todayStr: string, now: number = Date.now()): Counters {
  ...
  return { ...c, date: todayStr, nightOwl: 0, earlyBird: 0, focusMs, focusStartedAt, focusEndsAt, lastMood: null, lastMoodAt: null };
}
...
export interface SkinDef {
  ...
  /** How it's earned, shown as the locked tooltip / hint. */
...
export const SKIN_REGISTRY: SkinDef[] = [
  ...
  },
  {
    ...
    label: { en: 'Early Bird', th: 'บูตื่นเช้า' },
  ...
  // ── events ─────────────────────────────────────────────────────────────────,
  {
    ...
    label: { en: 'Jack Boo', th: 'บูฮาโลวีน' },
  ...
  {
    id: 'bunny', kind: 'event',
    label: { en: 'Spring Boo', th: 'บูอีสเตอร์' },
